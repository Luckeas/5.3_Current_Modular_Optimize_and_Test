...
from backtester_common import (
    ...
    calculate_position_size, reset_hmm_detector
)
...
def run_backtest(df, visualize_trades=True, file_paths=None, use_ml=None):
    ...
    for i, row in df.iterrows():
        ...
        if position != 0:
            # Call exit condition check with price tracking
            exit_result = check_exit_conditions(
                row, position, entry_price, stop_loss, profit_target, i - entry_bar,
        ...
        # CHECK FOR NEW ENTRY SIGNALS (if not already in a position)
            ...
            if ml_enabled:
                ...
                entry_signal = check_entry_signal(prev_row, current_market_type)
